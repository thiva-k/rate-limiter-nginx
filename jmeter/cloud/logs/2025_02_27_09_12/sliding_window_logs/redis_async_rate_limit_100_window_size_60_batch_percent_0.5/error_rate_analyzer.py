...
FILE_PATH = os.path.join(CURRENT_DIR, "jmeter_results.csv")
RATE_LIMIT = 100  # Maximum requests allowed per window
WINDOW_SIZE = 60  # Window size in seconds

# Output file names in current directory
ERROR_DETAILS_FILE = os.path.join(CURRENT_DIR, "error_request_details.csv")
CLIENT_STATS_FILE = os.path.join(CURRENT_DIR, "client_error_stats.csv")
OVERALL_STATS_FILE = os.path.join(CURRENT_DIR, "overall_error_stats.csv")
...
def save_and_print_results(client_stats, overall_stats, error_details):
    ...
    print(f"1. {os.path.basename(CLIENT_STATS_FILE)} - Per-client statistics")
    print(f"2. {os.path.basename(OVERALL_STATS_FILE)} - Overall statistics")
    if not error_details.empty:
        print(f"3. {os.path.basename(ERROR_DETAILS_FILE)} - Detailed error information")

if __name__ == "__main__":
    # Check if input file exists
    if not os.path.exists(FILE_PATH):
        print(f"Error: Input file '{os.path.basename(FILE_PATH)}' not found in the current directory.")
        exit(1)
