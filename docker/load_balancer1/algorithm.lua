...
local function rate_limit(red, token)
    ...
    -- Here we calculate token generated per milliseconds so we multiply by 1000
    local now = ngx.now() * 1000 -- Current timestamp in milliseconds
    local last_tokens = tonumber(results[1]) or (bucket_capacity * 1000)
    ...
    local new_tokens = math.min(bucket_capacity * 1000, last_tokens + add_tokens)

    -- Calculate TTL for the Redis keys
    local ttl = math.floor(bucket_capacity / refill_rate * 2)

    -- Calculate next rest time
    local next_reset = math.ceil((last_access / 1000) + (1 / refill_rate))

    -- Check if there are enough tokens for the request
    if new_tokens >= (requested_tokens * 1000) then
        -- Deduct tokens and update Redis state
        new_tokens = new_tokens - (requested_tokens * 1000)
        ...
        return true, "allowed", new_tokens / 1000, next_reset
    else
        -- Not enough tokens, rate limit the request
        return true, "rejected", new_tokens / 1000, next_reset
    end
end
...
local function main()
    ...
    local pcall_status, rate_limit_result, message, remaining_tokens, next_reset = pcall(rate_limit, red, token)
    ...
    if not rate_limit_result then
        ...
        ngx.header["X-RateLimit-Reset"] = next_reset
