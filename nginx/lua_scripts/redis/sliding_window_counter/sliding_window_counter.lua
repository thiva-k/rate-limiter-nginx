...
local function get_token()
    ...
    ngx.log(ngx.DEBUG, "Token fetched successfully: ", token)
    return token
end
...
local function get_current_window_key()
    ...
    ngx.log(ngx.DEBUG, "Current window key: ", current_window_key, ", Sub-window size: ", sub_window_size, ", Now: ", now)
...
local function get_total_requests(red, token, current_window_key, sub_window_size, now)
    ngx.log(ngx.DEBUG, "Computing total requests across sliding windows")
    ...
    ngx.log(ngx.DEBUG, "Current count: ", current_count, ", Elapsed time: ", elapsed_time)
    ...
    for i = 1, sub_window_count do
        ...
        ngx.log(ngx.DEBUG, "Sub-window ", i, ", Previous count: ", previous_count, ", Total requests: ", total_requests)
    end

    ngx.log(ngx.DEBUG, "Total requests across sliding windows: ", total_requests)
    return total_requests
end

-- Main rate limiting logic
local function check_rate_limit(red, token)
    ngx.log(ngx.DEBUG, "Executing rate limit check for token: ", token)
    ...
    if not total_requests then
        ngx.log(ngx.ERR, "Error computing total requests: ", err)
        return ngx.HTTP_INTERNAL_SERVER_ERROR, err
    end

    ngx.log(ngx.DEBUG, "Total requests: ", total_requests, ", Request limit: ", request_limit)
    ...
    ngx.log(ngx.DEBUG, "Incremented Redis counter: ", new_count)
    ...
    ngx.log(ngx.DEBUG, "Expiration set for key: rate_limit:" .. token .. ":" .. current_window_key)
...
local function main()
    ...
    if status == ngx.HTTP_OK then
        ngx.log(ngx.DEBUG, "Rate limit check passed")
        ngx.log(ngx.DEBUG, "\n")
        ngx.say(message)
        ngx.exit(status)
    else
        ...
        ngx.log(ngx.DEBUG, "Rate limit check failed")
        ngx.log(ngx.DEBUG, "\n")
        ngx.exit(status)
    end
end
