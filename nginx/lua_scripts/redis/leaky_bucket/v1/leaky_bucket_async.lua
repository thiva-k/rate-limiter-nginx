...
local requested_tokens = 1 -- Number of tokens required per request -- TODO: have to think about how to process when the requested_tokens is greater than 1
...
local function rate_limit()
    ...
    if batch_quota == 0 then
        ...
        local remaining_tokens, err = execute_rate_limit(red, sha, tokens_key, last_access_key, bucket_capacity, leak_rate, 0, ttl)
    ...
    if batch_used >= batch_quota then
        ...
        local remaining_tokens, err = execute_rate_limit(red, sha, tokens_key, last_access_key, bucket_capacity, leak_rate, batch_used, ttl)
        if not remaining_tokens then
            ngx.log(ngx.ERR, "Failed to run rate limiting script: ", err)
            lock:unlock() -- Release the lock
            ngx.exit(ngx.HTTP_INTERNAL_SERVER_ERROR)
